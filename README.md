...
The TypeScript SDK ([sdk/](sdk/), `@ai-pnl/sdk` - zero runtime dependencies, fail-open always) is how internal products report what no vendor console can see: `pnl.wrap(client)` counts every OpenAI/Anthropic call from the response usage fields (streaming included), and `pnl.track(kind, { value, ref, employee })` records a success and its value, with the request context's tokens attached automatically. Quickstarts for Next.js, Express, and plain scripts live in [sdk/README.md](sdk/README.md). Events carry client-side UUIDs, buffer locally (cap 10,000, oldest dropped), and flush every 5 seconds or 100 events; the server upserts on the UUID, so retries never double-count and SDK problems never break the host app.
