...
export function reportPdfLines(data: ReportData): PdfLine[] {
  ...
  lines.push({
    text: "Generated by AI P&L - every number drills to its source rows in the dashboard.",
    font: "H",
    size: 8,
    gapBefore: 16,
  });
  ...
  for (const to of config.recipients) {
    try {
      await sendEmail(
        {
          to,
          subject: `AI P&L monthly report - ${monthLabel(month)}`,
          text:
            `The AI P&L report for ${monthLabel(month)} is attached.\n\n` +
