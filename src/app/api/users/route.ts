...
/** Add a view-only user (spec 11: the admin can add view-only users). */
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO users (name, role, password_hash)
       VALUES ($1, 'viewer', $2) RETURNING id, name, role`,
      [name, await hashPassword(password)],
    );
    logger.info("viewer added", { userId: rows[0].id, by: admin.id });
